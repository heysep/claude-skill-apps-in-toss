...
### 배포 전 광고 ID가 번들에 실제로 들어갔는지 확인한다 (실사례: 구독 지킴이)

환경변수 주입 방식은 zero-footprint 덕분에 **ID 없이 빌드해도 아무 에러가 없다** — 광고만
조용히 사라진 채 배포된다. 구독 지킴이가 이렇게 광고 없는 빌드로 라이브에 나간 적이 있다.
재발 방지:

1. ID 발급 즉시 `.env.production`에 저장한다 (매 빌드 셸 환경변수에 의존하지 않는다)
2. `ait deploy` 전에 `dist`에서 ID 문자열을 grep으로 확인한다:
   `rg "ait.v2.live" dist/` → 발급한 ID가 나와야 통과
